...
# Adapted from https://github.com/XPixelGroup/HAT (CVPR 2023)
...
class OCAB(nn.Module):
    ...
    Args:
        ...
        input_resolution (tuple[int]): Input resolution.
        window_size (int): Window size.
        overlap_ratio (float): Overlap ratio. Default: 0.5.
        num_heads (int): Number of attention heads.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
    ...
    def forward(self, x, x_size):
        ...
        kv = qkv[1:].contiguous()  # 2, B, C, H, W

        # partition q into non-overlapping windows
        q = window_partition(q, self.window_size)  # nW*B, ws, ws, C
        q = q.view(-1, self.window_size * self.window_size, self.num_heads,
                    C // self.num_heads).permute(0, 2, 1, 3)  # nW*B, nH, ws*ws, head_dim

        # partition kv into overlapping windows
        kv = kv.view(2, B, C, H * W).contiguous()
        kv = kv.view(2 * B, C, H, W)
        kv = self.unfold(kv)  # 2B, C*ows*ows, nW
        kv = kv.view(2, B, C, self.overlap_win_size * self.overlap_win_size, -1)
        kv = kv.permute(0, 1, 4, 3, 2)  # 2, B, nW, ows*ows, C
        nW = kv.shape[2]
        kv = kv.contiguous().view(2, B * nW, self.overlap_win_size * self.overlap_win_size,
                                   self.num_heads, C // self.num_heads)
        kv = kv.permute(0, 1, 3, 2, 4)  # 2, nW*B, nH, ows*ows, head_dim
        k, v = kv[0], kv[1]
...
class RHAG(nn.Module):
    ...
    Args:
        ...
        resi_connection (str): Convolutional block before residual connection. Default: '1conv'.
        compress_ratio (int): Compress ratio for CAB. Default: 3.
        squeeze_factor (int): Squeeze factor for CAB. Default: 30.
        overlap_ratio (float): Overlap ratio for OCAB. Default: 0.5.
    """
...
@register('hat')
class HAT(nn.Module):
    r"""HAT: Hybrid Attention Transformer for Image Super-Resolution.

    A PyTorch impl of: `Activating More Pixels in Image Super-Resolution Transformer`
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=4,
        ...
        if self.upsampler == 'none':
            ...
            raise NotImplementedError('pixelshuffledirect not implemented for HAT encoder mode')
